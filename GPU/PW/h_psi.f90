...
SUBROUTINE h_psi( lda, n, m, psi, hpsi )
...
#if (defined(__CUDA) && !defined(__DISABLE_CUDA_VLOCPSI) && !defined(__PARA)) || (defined(__CUDA) && !defined(__DISABLE_CUDA_VLOCPSI) && defined(__PARA) && defined(__USE_3D_FFT))
        !
        ! NOTE: CUFFT and multiplan strategy
#if defined(__CUDA_MULTIPLAN_FFT)
        ierr =   vloc_psi_multiplan_cuda ( lda, dffts%nnr, dffts%nr1x, dffts%nr2x, dffts%nr3x, &
                                    n, m, psi, vrs(1,current_spin), hpsi, igk(1:), nls(1:), nlsm(1:), ngms, ngm)
#else
        !
        ! NOTE: *** official ***
  ...
  ELSE  
     ...
     ! NOTE: CUFFT and multiplan strategy
#if defined(__CUDA_MULTIPLAN_FFT)
     CALL vloc_psi_multiplan_cuda_k ( lda, dffts%nnr, dffts%nr1x, dffts%nr2x, dffts%nr3x, n, m, psi, vrs(1,current_spin), hpsi, igk(1:), nls(1:), ngms)
#else
     !
     ! NOTE: ***OFFICIAL*** this version does not include kernel_save_hpsi_k
